...
const mapStateToProps = state => ({ ...state.cashonline });
const mapDispatchToProps = dispatch => ({
  onGetOrders: (startDate, endDate, paymentType, data) =>
    dispatch({
      type: ORDERS_COD_ONLINE_LIST,
      startDate,
      endDate,
      paymentType,
      payload: AxiosRequest.Orders.getOrdersAmount(data)
    })
});
...
class CashOnlinePage extends React.Component {
  componentWillMount() {
    today = Moment(new Date()).format("YYYY-MM-DD");
    currentDate = Moment(new Date()).format("YYYY-MM-DD HH:MM:SS");
    startDate = this.props.startDate || today+' 07:00:00';
    endDate = this.props.endDate || currentDate;
    //startDate=endDate=Moment(new Date()).format("YYYY-MM-DD HH:MM:SS");
  ...
  getData() {
    data = {
      startdate: startDate,
      enddate: endDate,
      payment_type: paymentType===3?"":paymentType
    };
  ...
  render() {
    ...
    return (
        ...
        <Card>
          <CardHeader>
            <Row>
              ...
              <Col lg="2" className="txt-color-blue">
      ...
      // </PDFDownloadLink>)
              ...
              <Col lg="2" className="txt-align-right">
                 
                <ButtonGroup size="sm">
                <Button
                    color="primary"
                    onClick={() => this.filterPaymentType(3)}
                    active={this.props.paymentType === 3}
                  >
                    All
                  </Button>
                  <Button
