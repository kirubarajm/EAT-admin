...
const mapStateToProps = state => ({ ...state.dunzoorders });

const mapDispatchToProps = dispatch => ({
    onGetOrders: (data) =>
        dispatch({ type: DUNZO_ORDERS_ALL_LIST, payload: AxiosRequest.Eat.getDunzoOrders(data) }),
    onSetFilter: (orderType, search,selectedPage,moveitfilter) =>
        dispatch({ type: DUNZO_ORDERS_FILTER_LIST, orderType, search,selectedPage,moveitfilter }),
   
});
class DunzoOrderList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            selectedPage: 1,
...
export default connect(mapStateToProps, mapDispatchToProps)(DunzoOrderList);