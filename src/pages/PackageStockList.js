...
import { PACKAGE_STOCK_LIST_FILTER, PACKAGE_STOCK_LIST,
...
const mapStateToProps = state => ({ ...state.packagestocklist });

const mapDispatchToProps = dispatch => ({
  onGetAllPackageStock: data =>
    dispatch({
      type: PACKAGE_STOCK_LIST,
      payload: AxiosRequest.PackageInventory.getPackageStockInvetoryList(data)
    }),
  onSetFilter: (search, page) =>
    dispatch({ type: PACKAGE_STOCK_LIST_FILTER, search, page })
});

const defultPage = 1;

class PackageStockList extends React.Component {
  constructor() {
    super();
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(PackageStockList);
